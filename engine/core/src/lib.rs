...
/// rule, not a wire constant — the JS-facing `uploadTexture` small-texture
/// contract keeps its own pow2 `<= spec::TEX_MAX_DIM` rule untouched.
...
impl Ui {
    ...
    /// Register a NATIVELY produced image resource (host-side; there is no
    /// JS op for this — the guest only ever sees the returned
    /// generation-tagged handle plus caller-declared dimensions). This is
