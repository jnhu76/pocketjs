...
mod tests {
    ...
    fn a3_missing_file_pushes_bounded_error_and_harness_continues() {
        ...
        harness.observe_rx(
            &surface,
            &json!({"t": "a3open", "req": "r1", "path": missing}).to_string(),
        );
        harness.observe_rx(
            &surface,
            &json!({"t": "a3open", "req": "r2", "path": missing}).to_string(),
        );
        let lines = a3_sent(&harness);
    ...
    fn exif_orientation_transform_matches_the_exif_table() {
        ...
        // orientation semantics for every defined value.
        let table: [(u16, QuarterTurn, bool); 8] = [
            ...
            (5, QuarterTurn::R270, true),
            (6, QuarterTurn::R90, false),
            (7, QuarterTurn::R90, true),
    ...
    fn a3_open_request_decodes_registers_and_retires_previous_synchronously() {
        ...
        harness.observe_rx(
            &surface,
            &json!({"t": "a3open", "req": "r1", "path": path}).to_string(),
        );
        let lines = a3_sent(&harness);
        ...
        harness.observe_rx(
            &surface,
            &json!({"t": "a3open", "req": "r2", "path": path}).to_string(),
        );
        let lines = a3_sent(&harness);
