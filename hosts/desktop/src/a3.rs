...
mod wic {
    ...
    pub(super) fn decode_jpeg(bytes: &[u8]) -> Result<A3Decoded, A3DecodeError> {
        ...
        if transform.rotate != QuarterTurn::R0 || transform.flip_h {
            // Materialize the decoded plane as an IWICBitmap, then pull the
            // rotated/flipped plane out of memory.
            ...
            let (mut w_rot, mut h_rot) = (0u32, 0u32);
            unsafe { source.GetSize(&mut w_rot, &mut h_rot) }
                .map_err(|_| A3DecodeError::Corrupt)?;
            let plane_rot = super::checked_plane_bytes(w_rot, h_rot)?;
            let stride_rot = w_rot.checked_mul(4).ok_or(A3DecodeError::TooLarge)?;
            let mut pixels_rot = vec![0u8; plane_rot as usize];
            unsafe { source.CopyPixels(std::ptr::null(), stride_rot, &mut pixels_rot) }
                .map_err(|_| A3DecodeError::Corrupt)?;
            w_out = w_rot;
            h_out = h_rot;
            // The unrotated intermediate plane dies here, on the native
            // side; only the oriented plane reaches the seam.
            drop(pixels);
            pixels = pixels_rot;
        }
...
impl A3Harness {
        ...
        match value["t"].as_str() {
            Some("a3open") => {
                ...
                match value["path"].as_str() {
                    Some(path) => self.handle_open(surface, &req, path.into(), tick),
    ...
    fn handle_open(
        ...
        let all = Instant::now();
        eprintln!(
            "A3EVENT,open,req={req},path={},tick={tick},epochUs={}",
            path.display(),
            epoch_us()
        );
        ...
        let (decoded, decode_us) = {
            ...
            match decode_jpeg_wic(&bytes) {
                ...
                Err(error) => return self.announce_error(surface, req, error.code(), tick),
            }
        };
        ...
        self.successes += 1;
        eprintln!(
            "A3EVENT,img,req={req},handle={handle},w={w},h={h},orient={orientation},plane={plane},liveBytes={live_bytes},openUs={open_us},decodeUs={decode_us},registerUs={register_us},retiredReq={retired_req},totalUs={},tick={tick},epochUs={}",
